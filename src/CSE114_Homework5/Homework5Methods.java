...
public class Homework5Methods {
    ...
    public static int lowestBasePalindrome(int num) {
        ...
        for (int base = 2; base < num; base++) {
            ...
            convertFromBaseTen(num, base, myList);
    ...
    private static void convertFromBaseTen(int base10Num, int newBase, ArrayList myList) {

        while (base10Num != 0) {
            myList.add(base10Num % newBase);
            base10Num /= newBase;
        }
    }

    private static boolean isPalindrome(ArrayList<Integer> myList) {
        ...
        while (!myStack.isEmpty()) {
            if (!myStack.peek().equals(myList.get(i))) {
                return false;
            }
            myStack.pop();
            i++;
